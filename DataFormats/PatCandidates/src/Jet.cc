...
const std::vector<int>& Jet::algoFlav(const reco::FlavAlgo& algo) const{
  return jetFlavourInfo_.getAlgoFlav(algo);
}
...
/// return the GHS algorithm-derived flavour of the jet, using bitwise encoding (0 as N/A)
uint Jet::algoFlavCode(const reco::FlavAlgo& algo) const{
  return jetFlavourInfo_.getAlgoFlavCode(algo);
}
...
int8_t Jet::algoFlavLeading(const reco::FlavAlgo& algo) const{
  return jetFlavourInfo_.getAlgoFlavLeading(algo);
}
...
bool Jet::haveAlgoFlav(const reco::FlavAlgo& algo) const{
  return jetFlavourInfo_.haveAlgoFlav(algo);
}
