...
void MergedGenParticleProducer::produce(edm::Event& event, const edm::EventSetup& setup) {
  ...
  const unsigned int n =
      pruned_handle->size() + (packed_handle->size() - st1_dup_map.size()) + nPhotonsFromPrunedHadron + nLeptonsFromPrunedPhoton;
...
bool MergedGenParticleProducer::isLeptonFromPrunedPhoton(const reco::GenParticle& pk) const {
  if ((abs(pk.pdgId()) == 11 or abs(pk.pdgId()) == 13) and not (pk.statusFlags().fromHardProcess() or pk.statusFlags().isDirectTauDecayProduct() )) {
